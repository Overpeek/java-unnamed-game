...
public class SimplexNoise_octave {  // Simplex noise in 2D, 3D and 4D
  ...
  private static final float G4 = (float) ((5.0f-Math.sqrt(5.0f))/20.0f);
  ...
  public float noiseOctave(float xin, float yin, int octaves, float persistance, float lacunarity) {
	  ...
	  for (int i = 1; i < octaves + 1; i++) {
		  max += 1.0f * amplitude;
		  value += noise(xin * frequency, yin * frequency, i * 532.43f) * amplitude;
		  
		  amplitude *= persistance;
		  frequency *= lacunarity;
	  }
	  
	  value = Maths.map((float) value, (float) -max, (float) max, -1.0f, 1.0f);
